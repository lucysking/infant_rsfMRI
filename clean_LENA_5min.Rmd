...
lena_followup_cs_day1 <-
  ...
  dplyr::select(ID, day, lena_caregivers___1:lena_notes)
...
lena_day1 <- 
  ...
  dplyr::select(ID, age:percent_mother)
...
lena_5min_viz %>% 
  ...
  mutate(
    ...
    ID = dplyr::recode(
      ID,
      "1081" = "Lower Consistency, Higher Quantity",
      "21" = "Lower Quantity, Higher Consistency"
    )
  ) %>% 
  ggplot(aes(time_hours, CTC.Actual, color = ID)) +
  geom_step(direction = "vh", size = 1) +
  scale_x_continuous(breaks = seq.int(6, 20, 2)) +
  scale_color_npg() +
...
ggsave(
  "~/Box/lucy_king_files/BABIES/rsfMRI/LENA/figures/quantity_v_consistency.eps",
  width = 17.6,
  height = 16,
  units = "cm",
  dpi = 300
